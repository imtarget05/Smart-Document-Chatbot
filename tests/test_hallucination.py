...
class MockHallucinationDetector:
    ...
    def _extract_claims(self, answer: str) -> List[Claim]:
        sentences = [
            s.strip() for s in answer.replace(".", ".").split(".") if s.strip()
        ]
    ...
    def _check_grounding(self, claims: List[Claim], docs: List[Dict]) -> List[bool]:
        ...
        for claim in claims:
            ...
            found = any(
                doc_text in claim_lower or claim_lower in doc_text
...
KNOWN_DOCS = [
    {
        "id": "doc1",
        "content": "Chính sách đổi trả cho phép khách hàng đổi trả sản phẩm trong vòng 30 ngày kể từ ngày mua.",
    },
    {
        "id": "doc2",
        "content": "Quy định an toàn lao động yêu cầu tất cả công nhân phải đội mũ bảo hộ khi vào nhà xưởng.",
    },
    {
        "id": "doc3",
        "content": "Chính sách lương thưởng dựa trên hiệu suất quý, thưởng tối đa 3 tháng lương.",
    },
    {
        "id": "doc4",
        "content": "Quy trình vệ sinh máy móc phải được thực hiện hàng ngày trước khi ca làm việc bắt đầu.",
    },
    {
        "id": "doc5",
        "content": "Bảo mật thông tin khách hàng là ưu tiên hàng đầu, mọi vi phạm sẽ bị xử lý kỷ luật.",
    },
]
...
class TestHallucinationDetection:
    ...
    def test_hal03_partial_hallucination(self):
        ...
        answer = (
            "Chính sách đổi trả 30 ngày [doc1]. Lương thưởng dựa trên KPI 200% [doc3]."
        )
    ...
    def test_hal04_confidence_calibration(self):
        ...
        if correct_confs and wrong_confs:
            ...
            assert avg_correct > avg_wrong, (
                f"Confidence not calibrated: correct={avg_correct}, wrong={avg_wrong}"
            )
