...
class MockModelRegistry:
    ...
    def register_model(self, model_name: str, version: str, metrics: Dict) -> Optional[ModelVersion]:
        ...
        mv = ModelVersion(
            version=version, model_name=model_name, stage="Staging",
            metrics=metrics, created_at=datetime.now(timezone.utc).isoformat()
        )
...
class MockDriftDetector:
    ...
    def check_drift(self) -> Dict:
        ...
        if psi >= self.threshold:
            alerts.append({"type": "DATA_DRIFT", "psi": psi, "threshold": self.threshold})
...
class MockABTestManager:
    ...
    def create_experiment(self, exp_id: str, variants: List[Dict]):
        self.experiments[exp_id] = {"variants": variants, "created_at": datetime.now().isoformat()}
        self.results[exp_id] = []
    ...
    def get_metrics(self, exp_id: str, variant_id: str) -> Dict:
        results = [r for r in self.results.get(exp_id, []) if r.get("variant_id") == variant_id]
        if not results:
...
class MockPredictionLogger:
    ...
    def to_json(self) -> str:
        return json.dumps([{
            ...
            "latency_ms": l.latency_ms, "confidence": l.confidence
        } for l in self.logs])
...
class TestMLOps:
    ...
    def test_mlo02_model_promotion(self):
        ...
        mv = registry.register_model("rag-model", "v1.0", {
            "retrieval_accuracy": 0.88, "answer_correctness": 0.82, "hallucination_rate": 0.04
        })
    ...
    def test_mlo03_quality_gate_block(self):
        ...
        registry = MockModelRegistry()
        mv = registry.register_model("rag-model", "v-bad", {
            "retrieval_accuracy": 0.60, "answer_correctness": 0.50, "hallucination_rate": 0.15
        })
    ...
    def test_mlo04_retrain_trigger(self):
        ...
        assert retrain_triggered == True, "Should trigger retrain after 3 consecutive drops"
    ...
    def test_mlo05_mlflow_logging(self):
        ...
        for i in range(10):
            logger.log(f"query_{i}", f"answer_{i}", latency_ms=100 + i * 10, confidence=0.85 + i * 0.01)
    ...
    def test_mlo07_rollback_on_degradation(self):
        ...
        ab = MockABTestManager()
        ab.create_experiment("exp-rollback", [
            {"id": "control", "name": "Control", "weight": 0.5},
        ...
        for i in range(100):
            ...
            if variant["id"] == "control":
                ab.log_result("exp-rollback", {"variant_id": "control", "correct": random.random() < 0.85, "latency_ms": 1200})
            else:
                ab.log_result("exp-rollback", {"variant_id": "new", "correct": random.random() < 0.65, "latency_ms": 1500})

        control_metrics = ab.get_metrics("exp-rollback", "control")
        new_metrics = ab.get_metrics("exp-rollback", "new")

        winner = "control" if control_metrics["accuracy"] > new_metrics["accuracy"] else "new"
        assert winner == "control", f"Should rollback to control: control={control_metrics['accuracy']:.2%}, new={new_metrics['accuracy']:.2%}"

    def test_mlo08_prediction_logging(self):
        """TC-MLO-08: Prediction Logging — all fields captured."""
        logger = MockPredictionLogger()
        logger.log("What is policy?", "Return policy is 30 days", latency_ms=1250.5, confidence=0.92)
