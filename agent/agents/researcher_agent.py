...
class ResearcherAgent:
    ...
    async def run(self, state: AgentState) -> AgentState:
        ...
        for doc_id in document_ids[:3]:
            try:
                chunks = await self._search.hybrid_search(query, doc_id, top_k=3, use_bm25=True)
        ...
        doc_context  = "\n\n".join(
            f"[{c.get('document_name','doc')}]\n{c.get('text','')}" for c in doc_chunks[:5]
        )
        full_context = ""
        if web_context:
        ...
        if not full_context:
            ...
            state["final_answer"] = "💭 [No external sources found]\n\n" + response.content.strip()
            state["sources"]      = []
        ...
        sources = [
            {"document_name": r.get("title", "Web"), "chunk_text": r.get("content", "")[:300],
             "score": 1.0, "source_type": "web", "url": r.get("url", "")}
            for r in web_results
        ] + [
