...
def measure_single_task(task, run_number):
    ...
    print(f"   Doing the manual process now...")
    ...
    print(f"   Started at: {get_timestamp()}")
    print(f"   Using the dashboard now...")
...
def run_measurement():
    ...
    for task_id in task_ids:
        ...
        avg_dashboard = sum(r["dashboard_seconds"] for r in task_results) / len(task_results)
        avg_saved = sum(r["time_saved_seconds"] for r in task_results) / len(task_results)
    ...
    final_summary = {
        ...
        "overall_summary": {
            ...
            "percentage_faster": round((1 - total_dashboard / total_manual) * 100, 1) if total_manual > 0 else 0
        }
    }
    ...
    for item in summary_by_task:
        ...
        print(f"  Manual: {item['avg_manual_seconds']:.1f}s | Dashboard: {item['avg_dashboard_seconds']:.1f}s | Saved: {item['avg_time_saved_seconds']:.1f}s | Speedup: {item['avg_speedup_factor']:.1f}x")
    
    print(f"\n{'='*60}")
    print(f"OVERALL SUMMARY")
    print(f"{'='*60}")
    print(f"Total manual time:     {total_manual:.1f} seconds ({total_manual/60:.1f} minutes)")
    print(f"Total dashboard time:  {total_dashboard:.1f} seconds ({total_dashboard/60:.1f} minutes)")
    print(f"Total time saved:      {total_saved:.1f} seconds ({total_saved/60:.1f} minutes)")
