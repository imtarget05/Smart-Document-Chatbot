...
def ask_question(base_url: str, token: str, session_id: str,
                 document_id: int, question: str) -> dict[str, Any]:
...
def run_evaluation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for i, q in enumerate(questions, 1):
        ...
        result = ask_question(args.base_url, args.token, session_id,
                              args.document_id, q["question"])
    ...
    summary = {
        ...
        "retrieval_accuracy": round(len(retrieval_accurate) / max(len(successful), 1), 4),
