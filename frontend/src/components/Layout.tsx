...
export default function Layout() {
  ...
  const navItems = [
    { path: '/classic', label: '💬 Classic Chat', description: 'Document Q&A', roles: ['ALL'] },
    { path: '/dashboard', label: '📊 Dashboard', description: 'Overview', roles: ['ALL'] },
    { path: '/agent', label: '🤖 Agent Chat', description: 'Multi-agent', roles: ['ALL'] },
    { path: '/knowledge', label: '📚 Knowledge Base', description: 'Docs & playbooks', roles: ['ALL'] },
    { path: '/datasources', label: '🔌 Data Sources', description: 'Connectors', roles: ['ALL'] },
    { path: '/eightd', label: '🧰 8D Cases', description: 'Incidents', roles: ['ENGINEER', 'ADMIN'] },
    { path: '/evaluation', label: '📈 Evaluation Lab', description: 'Quality', roles: ['ENGINEER', 'ADMIN'] },
    { path: '/audit', label: '🧾 Audit Logs', description: 'Security', roles: ['ADMIN'] },
    { path: '/settings', label: '⚙️ Settings', description: 'Preferences', roles: ['ALL'] },
    { path: '/admin', label: '🛡️ Admin', description: 'Users', roles: ['ADMIN'] },
  ];
  ...
  const isActive = (path: string) => {
    if (path === '/classic') return location.pathname === '/' || location.pathname === '/classic';
  ...
  return (
    <div className="flex h-screen bg-gray-100 font-sans">
      {/* Sidebar */}
      <div className={`${sidebarOpen ? 'w-72' : 'w-0'} transition-all duration-300 overflow-hidden bg-white border-r border-gray-200 flex flex-col`}>
        <div className="p-4 border-b border-gray-200">
          <h1 className="text-lg font-bold text-gray-800 flex items-center gap-2 cursor-pointer" onClick={() => navigate('/')}>
            ...
            <div className="space-y-1">
              {filteredNavItems.map((item) => (
                ...
                >
                  ...
                  <div className={`mt-0.5 text-[10px] ${isActive(item.path) ? 'text-indigo-100' : 'text-gray-400'}`}>
                    {item.description}
                  </div>
                </button>
              ))}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center shadow-sm flex-shrink-0">
          ...
          <span className="ml-4 text-sm font-semibold text-gray-600">
            {navItems.find(item => isActive(item.path))?.description || 'Workspace'}
          </span>
        </div>
