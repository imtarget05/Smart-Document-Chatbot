    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{documents.length} documents total</p>
        </div>
      ...
      {/* Documents Grid */}
      {isLoading ? (
        <div className="rounded-xl border border-dashed border-gray-300 p-6 text-sm text-gray-500">Loading documents…</div>
      ) : filteredDocs.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-300 p-6 text-sm text-gray-500">
          {documents.length === 0 ? 'No documents uploaded yet. Upload a document to get started.' : 'No documents match your filters.'}
        </div>
      ) : (
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {filteredDocs.map((doc) => (
            <div key={doc.id} className={`rounded-2xl border p-4 shadow-sm transition ${
              selectedIds.includes(doc.id) ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200 bg-white'
            }`}>
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <h3 className="font-semibold text-gray-800 text-sm">{doc.fileName}</h3>
                </div>
                <span className={statusBadge(doc.status)}>{doc.status}</span>
              </div>
              <div className="mt-2 space-y-1">
                <div className="text-xs text-gray-500">
                  <span className="font-medium">Size:</span> {formatSize(doc.fileSize)} • <span className="font-medium">Type:</span> {doc.fileType?.toUpperCase()} • <span className="font-medium">Chunks:</span> {doc.chunkCount}
                </div>
                {doc.summary && (
