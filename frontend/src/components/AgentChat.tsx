...
const AGENT_MODE_DESCRIPTIONS: Record<AgentMode, string> = {
  ...
  engineering:
    "Analyze test reports, failures, root cause, corrective actions, and 8D reports",
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-2">
            ...
            <p className="text-xs text-center max-w-xs">
              Chọn chế độ agent phù hợp hoặc để <strong>Auto</strong> tự quyết
              định.
              {documentIds.length > 0 && (
                <>
                  {" "}
                  Đang dùng <strong>{documentIds.length}</strong> tài liệu.
                </>
              )}
            </p>
          </div>
            ...
            >
              ...
              {msg.role === "agent" && msg.agent_type && (
                <div className="flex items-center gap-2 mb-1">
                  ...
                  >
                    {AGENT_MODE_LABELS[msg.agent_type as AgentMode] ??
                      msg.agent_type}
                  </span>
                  ...
                  <div className="mt-2 rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-50 via-white to-violet-50 p-3 shadow-sm">
                    ...
                    <div className="relative ml-1 space-y-3 border-l border-indigo-200 pl-4">
                      {msg.timeline.map((step, idx) => {
                        const icon =
                          STEP_ICONS[step.name] ?? STEP_ICONS.default;
                        const statusKey =
                          step.status === "ok" ? "ok" : "pending";
                        const statusStyle =
                          STEP_STATUS_STYLES[statusKey] ??
                          STEP_STATUS_STYLES.default;
                        const dotStyle =
                          STEP_DOT_STYLES[statusKey] ?? STEP_DOT_STYLES.default;
                        ...
                        return (
                          ...
                          >
                            <span
                              className={`absolute -left-[1.3rem] top-2 h-2.5 w-2.5 rounded-full border-2 border-white shadow-sm ${dotStyle}`}
                            />
                            <div className="rounded-xl bg-gradient-to-r from-white via-indigo-50/80 to-violet-50/70 p-2.5 shadow-sm ring-1 ring-indigo-100 transition-all duration-200 group-hover:-translate-y-0.5 group-hover:shadow-md">
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2">
                                  <div
                                    className={`rounded-full px-2 py-0.5 text-[10px] font-medium ring-1 ${statusStyle}`}
                                  >
                                    {step.status === "ok"
                                      ? "Completed"
                                      : "Pending"}
                                  ...
                                  <span
                                    className={`text-sm text-indigo-500 transition-transform duration-300 ${isExpanded ? "rotate-180" : "rotate-0"}`}
                                  >
                                    ⌄
                                  </span>
                                </div>
                              ...
                              <div
                                className={`grid overflow-hidden transition-all duration-300 ease-out ${isExpanded ? "mt-2 max-h-24 opacity-100" : "mt-0 max-h-0 opacity-0"}`}
                              >
                                <div className="overflow-hidden rounded-lg border border-indigo-100 bg-white/80 px-2.5 py-2 text-[11px] leading-5 text-gray-600">
                                  ...
                                  <div className="mt-1 text-gray-500">
                                    ...
                                    step and review the workflow detail.
                                  </div>
                                </div>
                              </div>
                            </div>
