...
describe('DocumentUpload', () => {
  it('rejects an unsupported file before sending a request', async () => {
    ...
    await userEvent.upload(input, new File(['bad'], 'payload.exe', { type: 'application/octet-stream' }));

    expect(screen.getByText('Only PDF, DOCX, and TXT files are supported')).toBeInTheDocument();
