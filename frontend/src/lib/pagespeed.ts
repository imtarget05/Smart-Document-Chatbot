...
export async function checkPageSpeed(
  ...
  try {
    ...
    const opportunities: { title: string; description: string; impact: string }[] = [];
    ...
    if (lhr?.audits) {
      Object.entries(lhr.audits).forEach(([, audit]: [string, any]) => {
        ...
        if (audit.scoreDisplayMode === 'opportunistic' && audit.details?.items) {
          opportunities.push({
            title: audit.title,
  ...
  if ('PerformanceObserver' in window) {
    try {
      ...
      new PerformanceObserver((list) => {
        ...
        if (entries.length > 0) {
          ...
          window.dispatchEvent(new CustomEvent('web-vital', {
            detail: { name: 'LCP', value: lcp.startTime, rating: lcp.startTime < 2500 ? 'good' : lcp.startTime < 4000 ? 'needs-improvement' : 'poor' }
          }));
        }
      ...
      new PerformanceObserver((list) => {
        ...
        window.dispatchEvent(new CustomEvent('web-vital', {
          detail: { name: 'CLS', value: cls, rating: cls < 0.1 ? 'good' : cls < 0.25 ? 'needs-improvement' : 'poor' }
        }));
