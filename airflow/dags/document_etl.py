...
def embed_and_index_fn(**kwargs):
    ...
    for idx, chunk in enumerate(chunks):
        ...
        ollama_payload = {"model": OLLAMA_EMBEDDING_MODEL, "prompt": chunk}
    ...
    for k in range(0, len(points), batch_size):
        ...
        requests.put(
            upsert_url, json={"points": batch}, headers=qdrant_headers
        ).raise_for_status()

    print(f"Successfully upserted vectors for collection: {collection_id}")

    # Pass details to callback
    return {"collection_id": collection_id, "chunk_count": len(chunks)}
